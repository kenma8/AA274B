...
class IntersectionScenario(gym.Env):
    ...
    def reset(self):
        ...
        self.intersection_y = self.np_random.random()*MAP_HEIGHT/2. + MAP_HEIGHT/4.
        self.ego.center += Point(self.np_random.random() - 0.5, self.np_random.random()*(self.intersection_y-MAP_HEIGHT/10.))
        self.ego.velocity = Point(0, self.np_random.random()*5)
    ...
    def action_space(self):
        return Box(low=np.array([-0.5,-2.0]), high=np.array([0.5,1.5]), dtype=np.float32)
    ...
    def _get_obs(self):
        return np.array([self.ego.center.x, self.ego.center.y, self.ego.speed, self.ego.heading, self.intersection_y], dtype=np.float32)
